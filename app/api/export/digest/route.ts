...
export async function GET(request: Request) {
  ...
  try {
    const alerts = getAlertsList({ limit: 500, since: dayStart }).filter((a) => a.timestamp <= dayEnd);
    const events = getRecentEvents(1000).filter((e) => e.timestamp >= dayStart && e.timestamp <= dayEnd);
