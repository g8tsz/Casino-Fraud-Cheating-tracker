...
- **casinoId** – Optional; scopes event to a casino property (multi-tenant).
- **eventId** – Optional; idempotency key (duplicates skipped).
- **observedRtp** – Optional; for `win` events (legacy alias: `expectedRtp`).
- **gameType** – Optional; `slots`, `blackjack`, `roulette`, `poker`, `craps`, `baccarat`.
- **ip**, **deviceId**, **geo** – Optional; used for collusion/session anomaly rules.
- **shoeBetCount** – Optional; blackjack bet count in current shoe (card counting).
